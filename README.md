...
│                         (client.py CLI)                             │
│                                                                     │
│  1. Collects customer data (income, education, family, CD account)  │
│  2. Validates input (type checking, range validation)               │
│  3. Builds feature dictionary matching model requirements           │
└────────────────────────────┬────────────────────────────────────────┘
                             │ HTTP POST
...
│  │    • model/loan_xgb.joblib → XGBoost classifier               │  │
│  │    • model/model_meta.json → thresholds & feature names       │  │
│  └───────────────────────────────────────────────────────────────┘  │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │ 2. /predict ENDPOINT (POST)                                   │  │
│  │    • Validate request via Pydantic (schemas.py)               │  │
│  │    • Ensure all required features present                     │  │
│  │    • Build pandas DataFrame in correct column order           │  │
│  │    • Call model.predict_proba(X)                              │  │
│  │    • Apply tier thresholds:                                   │  │
│  │       - VIP Promo: p ≥ 0.8                                    │  │
│  │       - Standard: p ≥ 0.5                                     │  │
│  │       - High Recall: p ≥ 0.3                                  │  │
│  │    • Return JSON response with probability & decisions        │  │
│  └───────────────────────────────────────────────────────────────┘  │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐  │
...
└────────────────────────────┬────────────────────────────────────────┘
                             │ HTTP 200 Response
                             │ JSON: {probability, decisions, thresholds}
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│                    CLIENT DECISION LOGIC (client.py)                │
│                                                                     │
│  1. Parse API response                                              │
│  2. Display prediction probability to user                          │
│  3. Show which tier(s) qualified (VIP/Standard/High Recall)         │
│  4. Prompt: "Send email to this client? (Y/N)"                      │
│                                                                     │
│  IF YES → Call send_email.sendEmail(tier)                           │
...
└────────────────────────────┬────────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│                    EMAIL MODULE (send_email.py)                     │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │ 1. USER INPUT                                                 │  │
│  │    • Prompt for customer name                                 │  │
│  │    • Prompt for email (regex validation)                      │  │
│  └───────────────────────────────────────────────────────────────┘  │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │ 2. TEMPLATE LOADING                                           │  │
...
│  │    • Reads from /email_templates/ directory                   │  │
│  └───────────────────────────────────────────────────────────────┘  │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │ 3. SENDGRID INTEGRATION                                       │  │
│  │    • Load SENDGRID_API_KEY from environment                   │  │
│  │    • Validate API key format (must start with "SG.")          │  │
│  │    • Load SENDGRID_FROM_EMAIL (verified sender)               │  │
│  │    • Construct Mail object with:                              │  │
│  │      - from_email (verified sender)                           │  │
│  │      - to_emails (customer email)                             │  │
│  │      - subject ("Loan Offer!!")                               │  │
│  │      - html_content (tier-specific template)                  │  │
│  └───────────────────────────────────────────────────────────────┘  │
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │ 4. SEND & ERROR HANDLING                                      │  │
│  │    • Call sg.send(message)                                    │  │
│  │    • Check response status_code (202 = success)               │  │
│  │    • Log errors (auth failures, invalid templates, etc.)      │  │
...
│                         SENDGRID API                                │
│                                                                     │
│  • Receives email request with HTML content                         │
│  • Validates sender authentication                                  │
│  • Delivers personalized loan offer email to customer               │
│  • Returns 202 Accepted status on success                           │
└─────────────────────────────────────────────────────────────────────┘
